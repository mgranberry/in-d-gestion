...
var makeParser = function(outEmitter) {
    ...
    var parseBolus = function(item) {
        ...
        var wizardEvent = {
            type: 'wizard',
            recommended: {
                'carb':round10(parseFloat(item.FoodBolusSize), -2),
                'correction':round10(parseFloat(item.CorrectionBolusSize), -2)
            },
            bgInput: item.BG ? parseFloat(item.BG) : '',
            carbInput: parseFloat(item.CarbSize)*1000,
            insulinOnBoard: item.IOB ? parseFloat(item.IOB) : 0.0,
            insulinSensitivity: parseFloat(item.CorrectionFactor),
            bgTarget: parseInt(item.TargetBG),
            deviceTime: item.RequestDateTime,
            deviceId: baseEvent.deviceId
        };

        // the XML does not include the carb ratio. Calculate it here.
        if (parseFloat(item.FoodBolusSize) && parseFloat(item.CarbSize))
          wizardEvent.insulinCarbRatio = round10(parseFloat(item.CarbSize) * 1000 / parseFloat(item.FoodBolusSize), -1);

        if (baseEvent) {
            emitter.emit('data', baseEvent);
            wizardEvent.bolus = baseEvent;
        }
    ...
    var parseBasal = function(item) {
        ...
        // doses.
            emitter.emit('data', {
                ...
                duration: item.$.TempRateActivated ? item.BasalRate.$.Duration * 60000 : 86400000,
                deviceTime: item.EventDateTime,
                deviceId: PUMP_NAME,
